...
<div class="card shadow-sm">
    <div class="card-body">
            ...
            <div class="col-md-9">
                ...
                <select name="invoice_id" class="form-select">
                    ...
                    <option value="{{ $invoice->id }}" {{ optional($selectedInvoice)->id === $invoice->id ? 'selected' : '' }}>
                        {{ $invoice->invoice_number }} - {{ $invoice->guest?->full_name }} - Due ${{ number_format((float) $invoice->due_amount, 2) }}
                    </option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-3 d-flex align-items-end">
                <button class="btn btn-outline-primary w-100">Load Invoice</button>
            </div>
        </form>
        ...
        <form method="POST" action="{{ route('backend.payments.store') }}">
            ...
            <div class="mt-4">
                <button type="submit" class="btn btn-primary" {{ $selectedInvoice ? '' : 'disabled' }}>Register Payment</button>
                <a href="{{ route('backend.payments.index') }}" class="btn btn-outline-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
