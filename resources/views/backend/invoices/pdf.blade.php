...
<body>
    ...
    <p class="muted">Fecha de emisión: {{ $invoice->issue_date?->format('Y-m-d') }}</p>

    <p>
        <strong>Huésped:</strong> {{ $invoice->guest?->full_name ?? 'Sin huésped' }}<br>
        <strong>Reserva:</strong> {{ $invoice->booking?->booking_number ?? 'Sin reserva' }}<br>
        <strong>Estado:</strong> {{ ucfirst($invoice->status) }}
    </p>

    <table>
        <tbody>
            <tr><th>Cargos de habitación</th><td class="text-right">${{ number_format((float) $invoice->room_charges, 2) }}</td></tr>
            ...
            <tr><th>Impuesto</th><td class="text-right">${{ number_format((float) $invoice->tax_amount, 2) }}</td></tr>
            <tr class="total"><th>Total</th><td class="text-right">${{ number_format((float) $invoice->total_amount, 2) }}</td></tr>
            <tr><th>Pagado</th><td class="text-right">${{ number_format((float) $invoice->paid_amount, 2) }}</td></tr>
            <tr><th>Pendiente</th><td class="text-right">${{ number_format((float) $invoice->due_amount, 2) }}</td></tr>
        </tbody>
    </table>
